...
export default async (b) => {
	try {
		...
		if (!b && sessionStorage.xinfan_check) return;
		game.importedPack = true;
		const pList = ["https://proxy.aestarin.com/", "", "shturl.cc/e2lDZzleNYo", "https://hk.gh-proxy.com/", "https://tvv.tw/"];
		...
		sessionStorage.xinfan_check = true;
		...
		try {
			...
			localStorage.xinfan_clean = true
		}
		finally {
			prog.remove();
			alert('更新完成！');
